...
public class Main {
			...
			while(true) {
				...
				try (
					...
					if(requestParts.length != 3) {
						out.write(getHttpResponse(STATUSCODE_400, CONTENTTYPE_PLAIN, "Request with incorrect format!"));
					}
					else {
						...
						if (!method.equals("GET")) {
						    out.write(getHttpResponse(STATUSCODE_405, CONTENTTYPE_PLAIN, "Method is not supported."));
						    return;
						}
						
						String path = requestParts[1];	
						
						out.write(getFileContent(path));
					}
	...
	private static String getHttpResponse(String status, String contentType, String content) {
